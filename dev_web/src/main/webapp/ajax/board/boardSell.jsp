<%@ page language="java" contentType="text/html; charset=UTF-8"
	...
	처리 주체가 서버(was:Web Application Server)이다. ->뭘 쓰셨나요? 톰캣9.0
	동적페이지 처리가 가능하다 - back-end developer
	HTML, CSS, JS는 정적페이지 처리가 가능한 언어이다. - FRONT-END Developer
	vs-code에서 사용하던 live server는 사용이 불가한가요? 네
	왜냐하면 이 서버에는 jsp 엔진이 없어서 안된다.
	 -->
	 <%
	 String name = "scott";
	 out.print(name); // scott이라는 문자열이 브라우저에 출력될 것이다.
	 %>
<!DOCTYPE html>
<html>
<head>
...
<script>
	...
	$.ajax({
		...
		url: "./boardSellAction.jsp?"+new Date().getTime(),
				//boardsellAction을 연결해온다.
		success:function(result){
			//console.log("result:"+result); //result로 15를 가져옴
			//span 태그 아이디들을 제이쿼리로 불러와 상수로(이름같게) 만들어 준다.
