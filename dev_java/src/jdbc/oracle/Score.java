...
class ScoreFrame extends JFrame implements ActionListener, MouseListener {
	...
	ScoreFrame() {
		...
		jlName.setBorder(BorderFactory.createBevelBorder(0)); //네모 음영지게 하는 효과
		jlName.setBounds(10, 10, 120, 50);
		add(jtName = new JTextField()); //텍스트 필드 설정
		jtName.setFont(new Font("맑은 고딕", Font.BOLD, 20));
		jtName.setHorizontalAlignment(JTextField.LEFT); //텍스트 필드가 왼쪽으로 커서
		...
		jtKor.setHorizontalAlignment(JTextField.LEFT);
		...
		jtEng.setHorizontalAlignment(JTextField.LEFT);
		...
		jtMat.setHorizontalAlignment(JTextField.LEFT);
		...
		jbAdd.addActionListener(this); // 액션리스너 기능! 이버튼을 눌렀을때, 밑에 액션퍼폼드에서 수행해줘야함.
		...
		setResizable(true); //늘리고 줄일 수 있는 기능.
		setSize(1200, 300);
		setTitle("성적 관리프로그램");
		setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);//종료 프로그램
		setVisible(true);
	} // ScoreFrame 생성자 !!!

	@Override //액션 리스너를 수행하는 액션 퍼폼드. 이벤트 처리.
	public void actionPerformed(ActionEvent e) {
		String ButtonFlag = e.getActionCommand();
		
		if (ButtonFlag.equals("추가")) { //버튼 플래그가 추가와 같은 소스. 즉 추가를 눌렀을때.
			try {
				contentSet();
				int result = scoreDAO.insertScore(scoreDTO);
				if (result == 1) {
					JOptionPane.showMessageDialog(this, "추가 되었습니다."); //메세지창 띄우기
					jTableRefresh();
					contentClear();
				} // inner if
			} catch (Exception e2) {
				JOptionPane.showMessageDialog(this, "이름을 입력하세요!"); //그냥 추가,제거, 수정을 눌렀을때.
				e2.printStackTrace();
				return;
			} // try - catch
			...
			try {
				contentSet();
				int result = scoreDAO.deleteScore(scoreDTO); //숫자를 지우는 기능.
	...
	public void jTableSet() {
		...
		table.getTableHeader().setReorderingAllowed(false); // 칸이 따로 안떨어지게 해주는 기능.
		table.getTableHeader().setResizingAllowed(false); //사이즈조절 안되게 해줌.
		table.setSelectionMode(ListSelectionModel.SINGLE_INTERVAL_SELECTION);
		
		// 컬럼 정렬에 필요한 메서드
		DefaultTableCellRenderer celAlignCenter = new DefaultTableCellRenderer();
		celAlignCenter.setHorizontalAlignment(JLabel.CENTER); // 이름 ~ 국어점수 칸의 중간에 위치하게 정렬.
		DefaultTableCellRenderer celAlignRight = new DefaultTableCellRenderer();
		celAlignRight.setHorizontalAlignment(JLabel.RIGHT);
		DefaultTableCellRenderer celAlignLeft = new DefaultTableCellRenderer();
		celAlignLeft.setHorizontalAlignment(JLabel.LEFT);
		
		// 컬럼별 사이즈 조절 & 정렬 ->칼럼 6개 정렬
		//비슷한 방법으로 셀의 너비지정은 table객체의 .getColumn() 매소드로 해당 컬럼을 가져온뒤, 
		//.setPreferredWidth 매소드로 너비를  픽셀 단위로 지정하면된다.
