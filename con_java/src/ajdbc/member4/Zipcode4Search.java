...
public class Zipcode4Search extends JFrame 
implements FocusListener, ActionListener, MouseListener {
	...
	Member4Ship 		  ms    = null;
	// 생성자
	public Zipcode4Search() {}
	public Zipcode4Search(Member4Ship ms) {
		this.ms = ms;
	}
	// 화면처리부
	...
	public void refreshData(String dong) {
		...
		try {
			...
			for(int i=0;i<zipList.size();i++) { //디폴트테이블모델에 뿌려줌.
	...
	public static void main(String[] args) {
		Zipcode4Search zc = new Zipcode4Search();
		zc.initDisplay();
	}
	@Override
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(obj == jtf_dong || obj == jbtn_search) {
			...
			String user = jtf_dong.getText();// 역삼, 당산 동에서 엔터쾅
			refreshData(user); // 파라미터로 넘겨주세요.
		}
	...
	public void mousePressed(MouseEvent e) {
		...
		if(index.length == 0) {
			...
			this.dispose(); // 필요없으니 자식창 닫기. + 회원가입이 성공했을때도 닫음.
		}
	}
	@Override
