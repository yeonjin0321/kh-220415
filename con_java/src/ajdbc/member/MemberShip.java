...
public class MemberShip extends JFrame implements ActionListener, MouseListener {
	...
	public int memberInsert(MemberVO pmVO) {
		...
		sql.append("VALUES(seq_member_no.nextval,?,?,?,?,?)"); //물음표로 들어와 있음.
		try {
	...
	*************************************************************************/                
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(obj ==jbtn_zipcode) {
			...
			ZipcodeSearch zs = new ZipcodeSearch(this); // 원본을 넘겨주는것. this로 넘겨서,멤버십
			zs.initDisplay();
		}
			...
			if(result == 1) {
				...
				//가입 성공이 되면 가입 화면은 닫아주세요.
				this.dispose(); // 자식창을 닫아주는 기능.
