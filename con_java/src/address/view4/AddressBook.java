...
//				Integer id= (Integer)myTableModel.getValueAt(index[0], 0);
	...
	private void addActionPerformed(ActionEvent evt) {
		...
		//AddressBook에서 입력버튼이 눌리면, abook에서 이벤트감지를 하고, 
		//ModifyDialog에 set메소드 호출한다.
		//호출할 때 파라미터로는 사용자가 선택한 버튼의 이름, 화면을 보여줄까? true, AddressVO(왜냐면 새로입력하는거니까)
		//null, abook의 주소번지()
...
//				Integer id= (Integer)myTableModel.getValueAt(index[0], 0);
				...
				for(int i=0;i<vos.length;i++) {
					if(id == vos[i].getId()) {
						newVo = new AddressVO(vos[i].getName(),vos[i].getAddress(),vos[i].getTelephone()
								             ,vos[i].getGender(),vos[i].getRelationship(),vos[i].getRegistedate()
								             ,vos[i].getBirthday(), vos[i].getComments(), vos[i].getId());
					}
	...
	public void refreshData() {
		...
		//디폴트 생성자로 변경한 이유는 send 메소드의 파라미터로 넘길 수 있다.
