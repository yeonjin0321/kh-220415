...
//컴넌트들의 위치를 정해줍니다.
	 ...
	 * @param editable 이른인스턴스화 후 입력 컴포넌트에 쓰기를 허용할건가? 아님 읽기 전용으로 할건가를 결정함.
	 * 입력(수정가능) 수정(수정가능), 상세보기 (onlyread)
	 * 다이얼로그, setVisible(false) -> setVisible(true)
	 * @param vo
	 * @param abook
	 * ***********************************************************/
	...
	// 셋팅하는 메쏘드입니다.
	private void setEditable(boolean e) { //파라미터 boolean에 따라서 결정된다. false
		txtName.setEditable(e); // true가 오면 이름을 변경가능함, false가 오면 변경이 불가능함.
